...
function getMatcher(filters) {
  ...
  return function match(file) {
    var dirs = file.split('/');
    log.debug('%s: filtering start', file);
      ...
      for (var j = 0, filterLen = filters.length; j < filterLen; j++) {
        if (filters[j].match(name, is_dir)) {
          if (filters[j].is_include) {
            log.debug('%s: include filter %d matches (%s)', name, j, filters[j].re.source);
            break;
          } else {
            log.debug('%s: exclude filter %d matches (%s)', name, j, filters[j].re.source);
            return false;
          }
        }
      }
      if (j == filterLen) {
        log.debug('%s: no filter matches', name);
      }
    }
    log.debug('%s: included', name);
    return true;
  }
};
